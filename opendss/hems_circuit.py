...
def build_circuit():
    ...
    temp_dir = r".\data\sample\temp_loads" 
    ...
    for col in df_loads.columns:
        if col not in ['Time', 'Hour', 'Minute']: 
            temp_path = rf"{temp_dir}\temp_load_{col}.csv"
            (df_loads[col] / 1000.0).to_csv(temp_path, index=False, header=False)
            # npts=96 (一天96筆), minterval=15 (間隔15分鐘)
            cmd = f"New LoadShape.Shape_{col} npts=96 minterval=15 mult=(file={temp_path})"
...
# 如果設備全部都50W 24小時 會等於 0.8kw,0.5kw,0.7kw,0.55kw,總負載等於 2.55kw
# ==========================================
