...
Original file is located at
    https://colab.research.google.com/drive/1e3dRvhh8SLyzuwXTThI-IKF1OtbMj6Dr
"""

# -*- coding: utf-8 -*-
"""
粒子群演算法（PSO）應用於 HEMS / BESS 排程最佳化

此版本重點：
1. 保留原本「迭代前 vs 迭代後」比較功能。
2. 將程式分成：參數設定、資料讀取、目標函數、模擬函式、PSO 主流程、畫圖。
3. 新增較多簡單註解，方便閱讀與修改。
4. simulate_schedule() 已改成與 hems_objective() 使用相同停電邏輯，
   避免「目標函數」與「圖表/實際電費」計算方式不一致。
"""
...
# Google Drive 中的負載資料路徑
LOAD_DATA_PATH = "/content/drive/MyDrive/HEMS_load_data/daily_appliance_data_1hr_kWh.csv"
...
# ============================================================
def run_optimization_pipeline(data_path="HEMS_load_data.csv"):
    """
    執行完整的最佳化與模擬流程，並將所有圖表/摘要所需的數據打包回傳。
    """
    # 1. 讀取 24 小時負載資料 (改用傳入的相對路徑，不再掛載 Google Drive)
    p_fixed = load_fixed_load_data(data_path)
    ...
    # 備註：請確保您原本的 run_pso 有記錄每次迭代的歷史最優值
    ...
    # 【核心修改】將原本傳給 print_summary 與 plot_results 的所有輸入打包
    data_package = {
        "before_position": before_position,
        "after_position": after_position,
